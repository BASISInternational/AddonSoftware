...
rem --- Open Files
	...
	open_tables$[19]="IVC_WHSECODE",open_opts$[19]="OTA"
...
if dropship$<>callpoint!.getColumnData("POE_RECHDR.DROPSHIP") and cvs(callpoint!.getDevObject("dropship_whse"),2)<>"" then
	if dropship$="Y" then
		callpoint!.setColumnEnabled("POE_REDHDR.WAREHOUSE_ID",0)
		callpoint!.setColumnData("POE_RECHDR.WAREHOUSE_ID",str(callpoint!.getDevObject("dropship_whse")),1)
		gosub whse_addr_info
	endif

	if dropship$="N" then
		callpoint!.setColumnEnabled("POE_RECHDR.WAREHOUSE_ID",1)
		callpoint!.setColumnData("POE_RECHDR.WAREHOUSE_ID","",1)
		gosub whse_addr_info
		callpoint!.setFocus("POE_RECHDR.WAREHOUSE_ID",1)
	endif
endif
...
if callpoint!.getUserInput()=callpoint!.getDevObject("dropship_whse") and callpoint!.getColumnData("POE_RECHDR.DROPSHIP")<>"Y" then
...
dropship_shipto: rem --- get and display shipto from Sales Order if dropship indicated, and OE installed
	...
	callpoint!.setColumnData("POE_RECHDR.SHIPTO_NO",shipto_no$,1)
	if cvs(shipto_no$,3)=""
		gosub shipto_cust
	endif
...
fill_dropship_address:
	...
	callpoint!.setColumnData("<<DISPLAY>>.W_ZIP",rec.zip_code$,1)
return
...
if callpoint!.getColumnData("POE_RECHDR.DROPSHIP")="Y"
	...
	util.changeControlLabel(SysGUI!, callpoint!, "POE_RECHDR.WAREHOUSE_ID", dropship_to$+":")
else
	ship_to_whse$=Translate!.getTranslation("AON_SHIP-TO")+" "+Translate!.getTranslation("AON_WAREHOUSE")
	util.changeControlLabel(SysGUI!, callpoint!, "POE_RECHDR.WAREHOUSE_ID", ship_to_whse$+":")
endif
